...
from data.utils import img2tensor, scandir
...
def get_all_image_paths(dataset_dir):
    ...
    for root, dirs, files in os.walk(dataset_dir):
        for file in files:
            # 假设图片文件为常见格式，如 .jpg、.png 等
            if file.endswith(('.jpg', '.jpeg', '.png', '.tif')):
                file_path = os.path.join(root, file)
                all_image_paths.append(file_path)
    return all_image_paths

def paired_paths_from_folder_debug(folders, keys, filename_tmpl):
    ...
    input_paths = get_all_image_paths(input_folder)
    gt_paths = get_all_image_paths(gt_folder)
