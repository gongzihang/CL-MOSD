...
class Skip_AutoencoderKL(AutoencoderKL):
    # TODO add titled
    ...
    def encode(
        ...
        if self.use_slicing and x.shape[0] > 1:
            ...
            h,skip_feature_list = self.encoder(x)
        ...
        return AutoencoderKLOutput(latent_dist=posterior), skip_feature_list

    def _decode(self, z: torch.FloatTensor, skip_feature_list=None, return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
        if self.use_tiling and (z.shape[-1] > self.tile_latent_min_size or z.shape[-2] > self.tile_latent_min_size):
            return self.tiled_decode(z, return_dict=return_dict)

        z = self.post_quant_conv(z)
        dec = self.decoder(z,skip_feature_list)
    ...
    @apply_forward_hook
    def decode(
        self, z: torch.FloatTensor, skip_feature_list=None, return_dict: bool = True, generator=None
    ) -> Union[DecoderOutput, torch.FloatTensor]:
        """
        Decode a batch of images.
        ...
        if self.use_slicing and z.shape[0] > 1:
            ...
            decoded = self._decode(z,skip_feature_list).sample
...
if __name__ == "__main__":
    vae = Skip_AutoencoderKL.from_pretrained("/home/gongzihang/workspace/Final_mix/pretrained/sd", subfolder="vae")
    ...
    sample, skip_feature_list = vae.encode(input_img)
    latent = sample.latent_dist.sample() * vae.config.scaling_factor
    
    print(latent.shape)
    output_image = (vae.decode(latent /vae.config.scaling_factor, skip_feature_list).sample).clamp(-1, 1)
